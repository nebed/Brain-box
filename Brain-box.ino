...
#define SOL_ADC A1     // Solar panel side voltage divider is connected to pin A1
#define AC_ADC A2    // AC side voltage divider is connected to pin A2 
#define CURRENT_ADC A3  // LEM LT-25NP current sensor is connected to pin A3
#define AVG_NUM 10    // number of iterations of the adc routine to average the adc readings
#define BAT_MIN 20  // minimum battery voltage for 24V system
#define BAT_MAX 30  // maximum battery voltage for 24V system
#define BULK_CH_SP 28.8 // bulk charge set point for sealed lead acid battery // flooded type set it to 29.2V
#define FLOAT_CH_SP 27.2  //float charge set point for lead acid battery
#define LVD 21          //Low voltage disconnect setting for a 24V system
#define PWM_PIN 9         // pin-9 is used to control the charging MOSFET //the default frequency is 490.20Hz
#define LOAD_PIN 10       // pin-10 is used to control the load
#define SOL_GREEN_LED 3
#define LOAD_RED_LED 5
#define AC_YELLOW_LED 4
#define TRAFFICATOR 2
//--------------------------------------------------------------------------------------------------------------------------
...
pinMode(AC_YELLOW_LED,OUTPUT);
pinMode(LOAD_RED_LED,OUTPUT);
pinMode(TRAFFICATOR,OUTPUT);
...
//------------------------------------------------------------------------------------------------------
////////////////// READS AND AVERAGES THE ANALOG INPUTS (SOLAR VOLTAGE,BATTERY VOLTAGE,AC VOLTAGE)////////////////
//------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------
 void read_data(void) 
 {
    ...
    // Vout=Vin*R2/(R1+R2) => Vin = Vout*(R1+R2)/R2   R1=100 and R2=20
     solar_volt = read_adc(SOL_ADC)*0.00488*7.429;
     bat_volt   = read_adc(BAT_ADC)*0.00488*5.3; 
   ac_volt   = read_adc(AC_ADC)*0.00488*7.429;       
   solar_current = (read_adc(CURRENT_ADC)*.00488)*0.3; 
...
watts = solar_current * bat_volt; //Watts now
...
//------------------------------------------------------------------------------------------------------------
  void print_data(void) 
  {
    ...
    Serial.println("V");
  Serial.print("AC Charger Voltage: ");
...
void ac_charge(void)
{
  if (ac_volt > bat_volt && bat_volt <= bulk_charge_sp)
  {
     ac_status = 1;
     duty= 255;
...
void battery_percent(void)
{
  ...
  if (system_volt == 24)
  {
    if (bat_volt >= 11 && bat_volt <= 21)
    {
       bat_percent = 25 - (1.363636*(21-bat_volt));
    }
    else if (bat_volt > 21 && bat_volt <= 23)
    {
      bat_percent = 49 - (12* (23-bat_volt));
    }
    else if (bat_volt > 23 && bat_volt <= 25.6)
    {
      bat_percent = 74 - (12* (25.6-bat_volt));
    }
    else if (bat_volt > 25.6 && bat_volt <= 29.4)
    {
      bat_percent = 100 - (6.578* (29.4-bat_volt));
    }
    else
    {
  ...
  {
    if (bat_volt >= 5.5 && bat_volt <= 10.5)
    {
       bat_percent = 25 - (1.363636*(10.5-bat_volt));
    }
    else if (bat_volt > 10.5 && bat_volt <= 11.5)
    {
      bat_percent = 49 - (12* (11.5-bat_volt));
    }
    else if (bat_volt > 11.5 && bat_volt <= 12.8)
    {
      bat_percent = 74 - (12* (12.8-bat_volt));
    }
    else if (bat_volt > 12.8 && bat_volt <= 14.7)
    {
      bat_percent = 100 - (6.578* (14.7-bat_volt));
    }
    else
    {
    }
...
{
  ac_led();
  solar_led();           //Battery status led indication
...
void solar_led(void)
{
  ...
  {   
      leds_off_all();
      digitalWrite(SOL_GREEN_LED,HIGH);  // battery voltage is healthy
  } 
  else
  {
      leds_off_all();
      digitalWrite(SOL_GREEN_LED,LOW);  //battery is fully charged
  }
}
//----------------------------------------------------------------------------------------------------------------------
/////////////////////////////////////////////AC LED INDICATION/////////////////////////////////////////////////////
...
void ac_led(void)
{
  ...
  {   
      leds_off_all();
      digitalWrite(AC_YELLOW_LED,HIGH);  // battery voltage is healthy
  } 
  else
  {
      leds_off_all();
      digitalWrite(AC_YELLOW_LED,LOW);  //battery is fully charged
  }
}
//----------------------------------------------------------------------------------------------------------------------
/////////////////////////////////////////////LOAD LED INDICATION/////////////////////////////////////////////////////
...
void leds_off_all(void)
{
  ...
  digitalWrite(AC_YELLOW_LED,LOW);
  digitalWrite(LOAD_RED_LED, LOW);
}
//------------------------------------------------------------------------------------------------------
//////////////////////////TRAFFICATOR///////////////////////////////////////////////////////////////////
//------------------------------------------------------------------------------------------------------
void trafficator(void)
{
  digitalWrite(TRAFFICATOR,HIGH);
  delay(500);
  digitalWrite(TRAFFICATOR,LOW);
  delay(500);
  digitalWrite(TRAFFICATOR,HIGH);
  delay(500);
  digitalWrite(TRAFFICATOR,LOW);
